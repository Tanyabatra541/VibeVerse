    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if searchText.isEmpty {
            ...
            filteredUsers = users.filter { user in
                // Debug: Print user's parsed skills
                print("Parsed Skills for \(user.firstName): \(user.goodAtThings)")
                
                // Check if any skill matches the search text using case-insensitive comparison
                return user.goodAtThings.contains { skill in
                    skill.contains(lowercasedSearchText)
                }
            }
        }

        // Debug: Print filtered users
        print("Filtered Users: \(filteredUsers.map { $0.firstName })")
...
extension AllUsersViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.viewProfileTapped = {
            self.navigationController?.pushViewController(UserProfileController(userData: self.filteredUsers[indexPath.row]), animated: true)
        }
        return cell
    }
